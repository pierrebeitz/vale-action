...
function wasLineTouchedInPR(filename, line) {
    var _a, _b, _c;
    ...
    if (!fromSHA)
        return true;
    const lines = cache[filename + fromSHA] || touchedLines(filename, fromSHA);
    cache[filename + fromSHA] = lines;
    return lines.includes(line);
}
exports.wasLineTouchedInPR = wasLineTouchedInPR;
function touchedLines(filename, fromSHA) {
    try {
        ensureGitHistory();
        return (execa
        ...
        core.error(`Failed to diff ${filename}. Here's a low level error: ${e.stderr}`);
        return [];
    }
}
